...
int main()
{
    ...
    xil_printf("\r\nEnter spindle speed (0-3): ");
    scanf("%d", &spindleSpeed);

    xil_printf("\r\nEnter spindle direction (0 = backwards, 1 = forwards): ");
    scanf("%d", &spindleDir);

    xil_printf("\r\nEnable spindle? (0 = disable, 1 = enable): ");
    scanf("%d", &spindleEn);

    xil_printf("\r\nEnter stepper direction (0 = backwards, 1 = forwards): ");
    scanf("%d", &stepperDir);

    xil_printf("\r\nEnable stepper? (0 = disable, 1 = enable): ");
    scanf("%d", &stepperEn);
    ...
    while (1) {
        ...
        // adjust spindle speed
            ...
            xil_printf("\r\nEnter new spindle speed (0-3): ");
            scanf("%d", &spindleSpeed);

            config &= ~0x03;
            config |= (spindleSpeed & 0x03);

            xil_printf("Spindle speed updated!\r\n");
            XGpio_DiscreteWrite(&gpio, 1, config & GPIO_MASK);
